...
public class MenuBuilder<T extends AbstractContainerMenu, S extends Screen & MenuAccess<T>, P>
	...
	public MenuBuilder(AbstractRegistrate<?> owner, P parent, String name, BuilderCallback callback,
		MenuFactory<T> factory, NonNullSupplier<ScreenFactory<T, S>> screenFactory) {
		this(owner, parent, name, callback, (type, windowId, inv, $) -> factory.create(type, windowId, inv),
			screenFactory);
	}

	public MenuBuilder(AbstractRegistrate<?> owner, P parent, String name, BuilderCallback callback,
		ForgeMenuFactory<T> factory, NonNullSupplier<ScreenFactory<T, S>> screenFactory) {
	...
	protected @NonnullType MenuType<T> createEntry() {
		...
		if (this.factory == null) {
			...
			ret = new ExtendedScreenHandlerType<>(
	...
	@Override
	protected RegistryEntry<MenuType<?>, MenuType<T>> createEntryWrapper(
		DeferredHolder<MenuType<?>, MenuType<T>> delegate) {
