...
public class GlassPipeVisual extends AbstractBlockEntityVisual<StraightPipeBlockEntity> implements SimpleDynamicVisual {
	...
	public void beginFrame(Context ctx) {
		...
		for (Direction side : Iterate.directions) {
			...
			var sprites = FluidVariantRendering.getSprites(fluidStack.getVariant());
			if (sprites == null || sprites.length == 0 || sprites[0] == null)
				continue;
			TextureAtlasSprite stillTexture = sprites[0];
			TextureAtlasSprite flowTexture = sprites.length > 1 && sprites[1] != null ? sprites[1] : stillTexture;

			int color = FluidVariantRendering.getColor(fluidStack.getVariant(), level, pos);
			int blockLightIn = (light >> 4) & 0xF;
			int luminosity = Math.max(blockLightIn, fluid == Fluids.LAVA ? 15 : 0);
