...
public abstract class FilterItem extends Item implements MenuProvider, SupportsItemCopying {
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
		...
		if (!player.isShiftKeyDown() && hand == InteractionHand.MAIN_HAND) {
			if (!world.isClientSide && player instanceof ServerPlayer serverPlayer)
				serverPlayer.openMenu(new FilterMenuProvider(heldItem.copy()));
	...
	private class FilterMenuProvider implements ExtendedScreenHandlerFactory<ItemStack> {
		private final ItemStack heldItem;

		private FilterMenuProvider(ItemStack heldItem) {
		...
		@Override
		public Component getDisplayName() {
			return FilterItem.this.getDisplayName();
		}

		@Override
		public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
			return FilterItem.this.createMenu(id, inv, player);
		}
	}
}
